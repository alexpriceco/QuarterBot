...
//need to substring to get the values we want initialized
...
public function getCourses($major){
  switch ($major){
    case "cs" :
...
public function getCMPSBSCourses(){
  ...
  $CMPS132/W=new Course("CMPS 132/W:Computability and Computational Complexity", 7, 6, "DC");
...
public function combineSimilar($array){
    $fixed=false;
    while(!$fixed){
      $fixed=true;
      for ($x=1; $x<count($array); $x++){
...
public function quarterify($array){ //courses2 is sorted and similarified
